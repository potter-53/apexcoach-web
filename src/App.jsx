...
function FlowCard({ step, title, text }) {
  return (
    <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6 shadow-[0_18px_60px_rgba(0,0,0,0.18)]">
      <div className="mb-5 inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-[var(--accent)]/30 bg-[var(--accent)]/12 text-sm font-semibold text-[var(--accent)]">
        {step}
      </div>
      <h3 className="text-2xl font-semibold text-white">{title}</h3>
      <p className="mt-4 text-base leading-8 text-white/68">{text}</p>
    </div>
  );
}
...
function FeatureItem({ children }) {
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-3">
      <div className="mt-0.5 flex h-6 w-6 items-center justify-center rounded-full bg-[var(--accent)]/14">
        <Check size={14} className="text-[var(--accent)]" />
      </div>
      <p className="text-white/78">{children}</p>
    </div>
  );
}
...
function PhoneMock({ src, alt }) {
  return (
    <div className="mx-auto w-[280px] rounded-[38px] border border-white/12 bg-[#090d10] p-2.5 shadow-[0_30px_120px_rgba(0,0,0,0.45)]">
      <div className="relative overflow-hidden rounded-[30px] border border-white/8 bg-black">
...
function BrowserMock({ src, title }) {
  return (
    <div className="overflow-hidden rounded-[30px] border border-white/12 bg-[#0c1216] shadow-[0_30px_120px_rgba(0,0,0,0.45)]">
      <div className="flex items-center gap-2 border-b border-white/8 bg-white/[0.04] px-5 py-4">
        ...
        <div className="ml-3 rounded-full border border-white/8 bg-black/20 px-4 py-1 text-xs text-white/55">
          {title}
        </div>
      </div>
      ...
      <div className="grid gap-0 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="flex flex-col gap-4 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-5">
          <div className="rounded-2xl border border-white/8 bg-black/20 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-white/45">Workspace</p>
            <p className="mt-2 text-lg font-semibold text-white">Today&apos;s coaching board</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-white/8 bg-white/[0.04] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-white/45">Clients</p>
              <p className="mt-2 text-3xl font-semibold text-white">24</p>
            </div>
            <div className="rounded-2xl border border-white/8 bg-white/[0.04] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-white/45">Sessions</p>
              <p className="mt-2 text-3xl font-semibold text-white">11</p>
            </div>
          </div>
          <div className="rounded-2xl border border-[var(--accent)]/20 bg-[var(--accent)]/10 p-4">
...
function Modal({ open, onClose, title, text }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
      <div className="relative w-full max-w-xl rounded-[32px] border border-white/10 bg-[#081014] p-8 text-white shadow-[0_30px_120px_rgba(0,0,0,0.5)]">
        <button
          onClick={onClose}
          className="absolute right-4 top-4 flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/70 transition hover:bg-white/10 hover:text-white"
          aria-label="Close"
        >
          <X size={18} />
        </button>
        <div className="mb-5 inline-flex rounded-full border border-[var(--accent)]/30 bg-[var(--accent)]/12 px-4 py-2 text-sm font-semibold text-[var(--accent)]">
          APEX COACH
        </div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-white">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10 bg-[radial-gradient(circle_at_top_left,rgba(110,231,183,0.16),transparent_28%),radial-gradient(circle_at_top_right,rgba(56,189,248,0.12),transparent_22%),linear-gradient(180deg,#071014_0%,#09161b_38%,#0c1015_100%)]" />
      <div className="pointer-events-none fixed inset-x-0 top-0 -z-10 h-[420px] bg-[linear-gradient(180deg,rgba(255,255,255,0.05),transparent)]" />

      <header className="sticky top-0 z-50 border-b border-white/8 bg-[rgba(7,16,20,0.72)] backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          <a href="#top" className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs uppercase tracking-[0.22em] text-white/40">apexcoach.pt</p>
            </div>
          </a>
            ...
            <div className="hidden rounded-full border border-white/10 bg-white/5 p-1 sm:flex">
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${
                  lang === "pt" ? "bg-[var(--accent)] text-[#081014]" : "text-white/65"
                }`}
              >
                PT
              </button>
              <button
                onClick={() => setLang("en")}
                className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${
                  lang === "en" ? "bg-[var(--accent)] text-[#081014]" : "text-white/65"
                }`}
              >
                EN
              </button>
            ...
            <div className="flex flex-col gap-2">
              {navItems.map((item) => (
                <a
                  ...
                  className="rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-3 text-white/75"
                >
                  {item.label}
                </a>
              ))}
      ...
      <main id="top">
        <section className="mx-auto grid max-w-7xl gap-14 px-5 pb-20 pt-16 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:pb-28 lg:pt-24">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-8 text-5xl font-semibold leading-[1.02] text-white sm:text-6xl xl:text-7xl">
              <span className="block">{t.titleA}</span>
              <span className="block text-white">{t.titleB}</span>
              <span className="block text-[var(--accent)]">{t.titleC}</span>
            </h1>
            ...
            <div className="mt-12 grid gap-5 sm:grid-cols-2">
              <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--accent)]/12">
                  <Smartphone size={22} className="text-[var(--accent)]" />
                </div>
                <p className="text-sm uppercase tracking-[0.2em] text-white/45">{t.heroMobileTag}</p>
                <h2 className="mt-2 text-xl font-semibold">{t.heroMobileTitle}</h2>
                <p className="mt-3 leading-7 text-white/68">{t.heroMobileText}</p>
              </div>

              <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--accent)]/12">
                  <Laptop2 size={22} className="text-[var(--accent)]" />
                </div>
                <p className="text-sm uppercase tracking-[0.2em] text-white/45">{t.heroBrowserTag}</p>
                <h2 className="mt-2 text-xl font-semibold">{t.heroBrowserTitle}</h2>
                <p className="mt-3 leading-7 text-white/68">{t.heroBrowserText}</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative flex items-center justify-center">
            ...
            <div className="relative w-full max-w-2xl">
              <div className="absolute -left-2 top-8 z-20 hidden rounded-3xl border border-white/10 bg-[#081014]/85 px-4 py-3 shadow-[0_24px_80px_rgba(0,0,0,0.35)] md:block">
                <div className="flex items-center gap-3">
                  <Clock3 size={16} className="text-[var(--accent)]" />
                  <div>
                    <p className="text-xs uppercase tracking-[0.2em] text-white/40">Session mode</p>
                    <p className="text-sm font-medium text-white/80">Fast changes, zero friction</p>
                  </div>
                </div>
              </div>

              <div className="absolute -right-2 bottom-10 z-20 hidden rounded-3xl border border-white/10 bg-[#081014]/85 px-4 py-3 shadow-[0_24px_80px_rgba(0,0,0,0.35)] md:block">
                <div className="flex items-center gap-3">
                  <LayoutDashboard size={16} className="text-[var(--accent)]" />
                  <div>
                    <p className="text-xs uppercase tracking-[0.2em] text-white/40">Desktop view</p>
                    <p className="text-sm font-medium text-white/80">Planning with full visibility</p>
                  </div>
                </div>
              </div>
        ...
        <section id="platform" className="border-y border-white/8 bg-white/[0.03]">
          <div className="mx-auto max-w-7xl px-5 py-20 lg:px-8">
            <div className="max-w-3xl">
              ...
              <p className="mt-6 max-w-2xl text-lg leading-8 text-white/68">{t.platformText}</p>
            </div>

            <div className="mt-12 grid gap-6 lg:grid-cols-2">
              <div className="rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))] p-7">
                <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-[20px] bg-[var(--accent)]/14">
                  <MonitorPlay size={26} className="text-[var(--accent)]" />
                </div>
                <h3 className="text-2xl font-semibold text-white">{t.phoneCardTitle}</h3>
                <p className="mt-4 max-w-xl leading-8 text-white/70">{t.phoneCardText}</p>
              </div>

              <div className="rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))] p-7">
                <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-[20px] bg-[var(--accent)]/14">
                  <Laptop2 size={26} className="text-[var(--accent)]" />
                </div>
                <h3 className="text-2xl font-semibold text-white">{t.browserCardTitle}</h3>
                <p className="mt-4 max-w-xl leading-8 text-white/70">{t.browserCardText}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="browser" className="border-y border-white/8 bg-white/[0.03]">
          <div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 lg:grid-cols-[0.95fr_1.05fr] lg:px-8">
            <div>
              ...
              <p className="mt-6 max-w-2xl text-lg leading-8 text-white/68">{t.browserText}</p>
        ...
        <section id="plans" className="border-y border-white/8 bg-white/[0.03]">
          <div className="mx-auto max-w-7xl px-5 py-20 lg:px-8">
            <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr]">
              <div>
                ...
                <p className="mt-6 max-w-2xl text-lg leading-8 text-white/68">{t.plansText}</p>
              </div>

              <div className="rounded-[32px] border border-[var(--accent)]/20 bg-[linear-gradient(180deg,rgba(110,231,183,0.15),rgba(110,231,183,0.06))] p-8">
                <div className="mb-5 inline-flex rounded-full border border-white/10 bg-black/15 px-4 py-2 text-sm text-white/72">
                  apexcoach.pt
                </div>
                <div className="grid gap-4">
                  <div className="rounded-2xl border border-white/10 bg-[#081014]/45 p-5">
                    <p className="text-sm uppercase tracking-[0.2em] text-white/45">Primary promise</p>
                    <p className="mt-2 text-xl font-semibold text-white">
                      The app stays at the core. The web adds premium clarity and control.
                    </p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-[#081014]/45 p-5">
                    <p className="text-sm uppercase tracking-[0.2em] text-white/45">CTA direction</p>
                    <p className="mt-2 text-white/78">
                      Trial, login web, and app download should live in the same conversion path.
                    </p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-[#081014]/45 p-5">
                    <p className="text-sm uppercase tracking-[0.2em] text-white/45">Perception shift</p>
                    <p className="mt-2 text-white/78">
        ...
        <section className="mx-auto max-w-5xl px-5 py-20 text-center lg:px-8">
          <div className="rounded-[36px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))] px-6 py-12 sm:px-10">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-[24px] bg-[var(--accent)]/14">
      ...
      <footer className="border-t border-white/8">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-5 py-8 text-sm text-white/45 lg:flex-row lg:px-8">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="APEX COACH" className="h-8 w-auto rounded-lg" />
            <div>
              <p className="font-medium text-white/80">APEX COACH</p>
              <p className="text-xs uppercase tracking-[0.18em] text-white/35">apexcoach.pt</p>
            </div>
          </div>

          <p className="text-center text-xs uppercase tracking-[0.18em] text-white/32">
