...
function AgendaCards({ items, onCreate, locale }) {
  const copy = getCopy(locale);
  return <SectionCard eyebrow={copy.agendaSpotlight} title={copy.agendaHeadline} description={copy.bookFasterText} action={<button onClick={onCreate} className="inline-flex items-center gap-2 rounded-2xl bg-[var(--accent)] px-4 py-3 text-sm font-semibold text-[var(--accent-foreground)]"><Plus size={16} />{copy.newBooking}</button>}>{items.length > 0 ? <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">{items.slice(0, 1).map((item) => <div key={item.id} className="rounded-[28px] border border-[var(--border)] bg-[linear-gradient(135deg,var(--accent-soft),rgba(124,77,255,0.08))] p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.18em] text-[var(--accent)]">{formatDate(item.scheduled_at, true, locale)}</p><p className="mt-2 text-4xl font-semibold text-[var(--text)]">{formatTime(item.scheduled_at, locale)}</p></div><span className="rounded-full border border-[var(--border)] bg-white px-3 py-1 text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">{prettifyStatus(item.status)}</span></div><div className="mt-8 flex items-center gap-3"><span className="h-3 w-3 rounded-full" style={{ background: colorDot(item.students?.client_color_hex) }} /><p className="text-lg font-semibold text-[var(--text)]">{item.students?.full_name || copy.client}</p></div><p className="mt-3 text-sm uppercase tracking-[0.16em] text-[var(--text-muted)]">{item.booking_types?.name || item.item_type || copy.bookingType}</p><p className="mt-4 max-w-xl leading-7 text-[var(--text-muted)]">{item.notes || copy.notesPlaceholder}</p></div>)}<div className="grid gap-4">{items.slice(1, 5).map((item) => <div key={item.id} className="rounded-[24px] border border-[var(--border)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(246,248,247,0.96))] p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.18em] text-[var(--accent)]">{formatDate(item.scheduled_at, true, locale)}</p><p className="mt-2 text-2xl font-semibold text-[var(--text)]">{formatTime(item.scheduled_at, locale)}</p></div><span className="rounded-full border border-[var(--border)] bg-white px-3 py-1 text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">{prettifyStatus(item.status)}</span></div><div className="mt-5 flex items-center gap-3"><span className="h-3 w-3 rounded-full" style={{ background: colorDot(item.students?.client_color_hex) }} /><p className="font-semibold text-[var(--text)]">{item.students?.full_name || copy.client}</p></div><p className="mt-2 text-sm uppercase tracking-[0.16em] text-[var(--text-muted)]">{item.booking_types?.name || item.item_type || copy.bookingType}</p></div>)}</div></div> : <EmptyState title={copy.noUpcomingTitle} text={copy.noUpcomingText} />}</SectionCard>;
}
