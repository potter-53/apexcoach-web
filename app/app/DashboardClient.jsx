...
function AgendaCards({ items, onCreate, locale }) {
  const copy = getCopy(locale);
  return <SectionCard eyebrow={copy.agendaSpotlight} title={copy.agendaHeadline} description={copy.bookFasterText} action={<button onClick={onCreate} className="inline-flex items-center gap-2 rounded-2xl bg-[var(--accent)] px-4 py-2.5 text-sm font-semibold text-[var(--accent-foreground)]"><Plus size={15} />{copy.newBooking}</button>}>{items.length > 0 ? <div className="grid gap-3 xl:grid-cols-[1.05fr_0.95fr]">{items.slice(0, 1).map((item) => <div key={item.id} className="rounded-[22px] border border-[var(--border)] bg-[linear-gradient(135deg,var(--accent-soft),rgba(124,77,255,0.08))] p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-[11px] uppercase tracking-[0.16em] text-[var(--accent)]">{formatDate(item.scheduled_at, true, locale)}</p><p className="mt-1 text-3xl font-semibold text-[var(--text)]">{formatTime(item.scheduled_at, locale)}</p></div><span className="rounded-full border border-[var(--border)] bg-white px-2.5 py-1 text-[10px] uppercase tracking-[0.16em] text-[var(--text-muted)]">{prettifyStatus(item.status)}</span></div><div className="mt-5 flex items-center gap-3"><span className="h-3 w-3 rounded-full" style={{ background: colorDot(item.students?.client_color_hex) }} /><p className="font-semibold text-[var(--text)]">{item.students?.full_name || copy.client}</p></div><p className="mt-2 text-xs uppercase tracking-[0.14em] text-[var(--text-muted)]">{item.booking_types?.name || item.item_type || copy.bookingType}</p>{item.notes ? <p className="mt-3 line-clamp-2 text-sm leading-6 text-[var(--text-muted)]">{item.notes}</p> : null}</div>)}<div className="grid gap-3">{items.slice(1, 5).map((item) => <div key={item.id} className="rounded-[18px] border border-[var(--border)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(246,248,247,0.96))] p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-[11px] uppercase tracking-[0.16em] text-[var(--accent)]">{formatDate(item.scheduled_at, true, locale)}</p><p className="mt-1 text-xl font-semibold text-[var(--text)]">{formatTime(item.scheduled_at, locale)}</p></div><span className="rounded-full border border-[var(--border)] bg-white px-2.5 py-1 text-[10px] uppercase tracking-[0.16em] text-[var(--text-muted)]">{prettifyStatus(item.status)}</span></div><div className="mt-3 flex items-center gap-3"><span className="h-3 w-3 rounded-full" style={{ background: colorDot(item.students?.client_color_hex) }} /><p className="font-medium text-[var(--text)]">{item.students?.full_name || copy.client}</p></div><p className="mt-1 text-xs uppercase tracking-[0.14em] text-[var(--text-muted)]">{item.booking_types?.name || item.item_type || copy.bookingType}</p></div>)}</div></div> : <EmptyState title={copy.noUpcomingTitle} text={copy.noUpcomingText} />}</SectionCard>;
}
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
        ...
        <div className="mx-auto grid min-h-screen max-w-[1600px] gap-5 px-4 py-4 lg:grid-cols-[260px_minmax(0,1fr)] lg:px-6">
          <aside className="rounded-[28px] border border-[var(--border-strong)] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(245,245,245,0.95))] p-4 shadow-[var(--shadow-panel)]"><div className="flex items-center gap-3"><div className="rounded-2xl border border-[var(--border)] bg-[linear-gradient(135deg,var(--accent-soft),rgba(124,77,255,0.08))] p-2"><LayoutDashboard size={18} className="text-[var(--accent-strong)]" /></div><div><p className="text-sm font-semibold tracking-[0.18em] text-[var(--text)]">APEX COACH</p><p className="text-[11px] uppercase tracking-[0.2em] text-[var(--text-muted)]">{copy.webWorkspace}</p></div></div><div className="mt-6 rounded-[22px] border border-[var(--border)] bg-[linear-gradient(135deg,var(--accent-soft),rgba(124,77,255,0.08))] px-4 py-3"><p className="text-sm font-semibold text-[var(--text)]">{coachName}</p><p className="mt-1 text-xs uppercase tracking-[0.16em] text-[var(--text-muted)]">{prettifyStatus(core.subscription?.status || "trialing")}</p></div><nav className="mt-6 grid gap-2">{appTabs.map(({ id, label, icon: Icon }) => { const active = activeTab === id; const emphasized = id === "agenda"; return <button key={id} onClick={() => startTransition(() => setActiveTab(id))} className={`flex items-center gap-3 rounded-[18px] border px-3 py-2.5 text-left text-sm transition ${active ? "border-[var(--accent)] bg-[var(--accent)] text-[#081014] shadow-[0_14px_28px_rgba(42,208,125,0.18)]" : emphasized ? "border-[var(--accent)]/25 bg-[linear-gradient(135deg,var(--accent-soft),rgba(124,77,255,0.06))] text-[var(--text)]" : "border-[var(--border)] bg-[var(--surface-solid)] text-[var(--text-muted)] hover:bg-[var(--surface-muted)]"}`}><span className={`flex h-8 w-8 items-center justify-center rounded-2xl ${active ? "bg-white/22" : "bg-white"}`}><Icon size={15} /></span><span className="font-medium">{label}</span></button>; })}</nav><div className="mt-6 grid gap-2 border-t border-[var(--border)] pt-4"><button onClick={openBookingModal} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[var(--accent)] px-4 py-2.5 text-sm font-semibold text-[var(--accent-foreground)]"><Plus size={15} />{copy.newBooking}</button><button onClick={handleSignOut} disabled={signingOut} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-[var(--border)] bg-white px-4 py-2.5 text-sm text-[var(--text-muted)] disabled:opacity-60">{signingOut ? <LoaderCircle size={15} className="animate-spin" /> : <LogOut size={15} />}{copy.signOut}</button><Link href="/" className="rounded-2xl px-3 py-2 text-center text-sm text-[var(--text-muted)]">{copy.backToLanding}</Link></div></aside>
          <section className="grid gap-4"><header className="rounded-[24px] border border-[var(--border-strong)] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(245,245,245,0.95))] p-4 shadow-[var(--shadow-panel)]"><div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-[var(--accent)]">{copy.coachBrowserWorkspace}</p><h1 className="mt-2 text-2xl font-semibold tracking-tight text-[var(--text)] sm:text-3xl">{copy.agendaHeadline}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--text-muted)]">{copy.agendaSubhead}</p></div><Link href="/login" className="inline-flex items-center justify-center rounded-2xl border border-[var(--border)] bg-white px-4 py-2.5 text-sm font-semibold text-[var(--text)]">{copy.switchAccount}</Link></div></header>{workspaceError ? <div className="rounded-[20px] border border-rose-200 bg-rose-50 px-5 py-4 text-rose-700 shadow-[var(--shadow-soft)]">{workspaceError}</div> : null}{loadingCore ? <div className="inline-flex items-center gap-3 rounded-full border border-[var(--border)] bg-[var(--surface-solid)] px-5 py-3 shadow-[var(--shadow-soft)]"><LoaderCircle size={18} className="animate-spin text-[var(--accent)]" />{copy.loadingCore}</div> : null}<div className="flex gap-3 overflow-x-auto pb-1 lg:hidden">{appTabs.map(({ id, label }) => <button key={id} onClick={() => startTransition(() => setActiveTab(id))} className={`shrink-0 rounded-full border px-4 py-2 text-sm font-medium ${activeTab === id ? "border-[var(--accent)] bg-[var(--accent)] text-[var(--accent-foreground)]" : "border-[var(--border)] bg-white text-[var(--text-muted)]"}`}>{label}</button>)}</div>
